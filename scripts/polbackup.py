...
def GeneratePollutionMapRelativeToGraph(graph, csvpath, pollutionheader='PM10'):
    ...
    for i in range(data.shape[0]):
        nearestnode = graph.ComputeNearestPoint(( data['longitude'][i], data['latitude'][i]))
        c = polmap.get(nearestnode, 0)
        polmap[nearestnode] = (c + data[pollutionheader][i])/2 # on fait la moyenne de la valeur actuelle et de la nouvelle
